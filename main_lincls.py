...
from utils.nvlars import LARC
import math
from setstim.resnet import resnet50
...
saved_path = os.path.join("exp/SetSim_sym2x_r50_200ep_lincls_sgd_bs256lr30/")
if not os.path.exists(saved_path):
    os.makedirs(saved_path)
logger = get_logger(os.path.join(saved_path, "linear_eval.log"))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            # rename 'encoder.' keys and delete projector & predictor
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.start_epoch and args.rank % ngpus_per_node == 0:
            sanity_check(model.state_dict(), args.pretrained)
        ...
        if epoch >= int(0.8 * args.epochs):
            ...
            if not args.multiprocessing_distributed or (
                ...
                save_checkpoint(
                    ...
                    is_best,
                    filename=saved_path + "checkpoint_{:04d}.pth.tar".format(epoch),
                )
...
def sanity_check(state_dict, pretrained_weights):
    ...
    for k, k_pre in zip(list(state_dict.keys()), list(state_dict_pre.keys())):
        ...
        k_pre = (
            "module.encoder_q.0." + k[len("module.") :]
